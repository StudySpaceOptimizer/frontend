...
-- 限制非管理員的更新範圍
CREATE OR
REPLACE FUNCTION cls_user_profiles_update () RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR
REPLACE FUNCTION set_user_metadata () RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    ...
    -- 設置 is_verified
    NEW.raw_app_meta_data := NEW.raw_app_meta_data || jsonb_build_object('is_verified', adminrole);
...
CREATE OR
REPLACE FUNCTION create_user_profile () RETURNS TRIGGER AS $$
BEGIN
...
-- 處理違規點數到達上限
CREATE OR
REPLACE FUNCTION ban_user_on_points_limit () RETURNS TRIGGER AS $$
DECLARE
 ...
 */
CREATE OR
REPLACE FUNCTION get_user_data (
