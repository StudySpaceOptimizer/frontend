...
CREATE POLICY admin_all_access ON seat_reservations AS PERMISSIVE FOR ALL USING (is_claims_admin ())
WITH
...
CREATE POLICY select_policy ON seat_reservations AS PERMISSIVE FOR
SELECT
    USING (true);
...
CREATE
OR REPLACE FUNCTION sync_seat_reservations () RETURNS TRIGGER AS $$
BEGIN
 ...
 */
CREATE
OR REPLACE FUNCTION get_seat_active_reservations (p_seat_id INT DEFAULT NULL) RETURNS TABLE (
    -- reservation
    id UUID,
